...
{
    float cameraMoveSpeed = 500.0f * deltaTime;   //20.0f for space scene
    float lightMoveSpeed = 5.0f * deltaTime;
    float baseMoveSpeed = 500.0f * deltaTime;
    ...
    if (cGFLWKeyboardModifiers::areAllModsUp(pWindow))
    {
        ...
        if (glfwGetKey(pWindow, GLFW_KEY_RIGHT) == GLFW_PRESS)
        {
            if (::canWeMove)
            {
                ::g_pPlayer->setPosition(glm::vec3(::g_pPlayer->getPosition().x - baseMoveSpeed, ::g_pPlayer->getPosition().y, ::g_pPlayer->getPosition().z));
                if (::g_pPlayer->getPosition().x <= -2000.0f)
                {
                    ::g_pPlayer->setPosition(glm::vec3(-2000.0f, ::g_pPlayer->getPosition().y, ::g_pPlayer->getPosition().z));
                }
            }
        }
        ...
        {
            if (::canWeMove)    // have to wait until the game starts to move
            {
                ::g_pPlayer->setPosition(glm::vec3(::g_pPlayer->getPosition().x + baseMoveSpeed, ::g_pPlayer->getPosition().y, ::g_pPlayer->getPosition().z));
                if (::g_pPlayer->getPosition().x >= 1500.0f)
                {
                    ::g_pPlayer->setPosition(glm::vec3(1500.0f, ::g_pPlayer->getPosition().y, ::g_pPlayer->getPosition().z));
                }
            }
        }
       ...
       // const float TERRAIN_MAP_DISPLACEMENT_ROTATION_SPEED = 0.025f;
        ...
        strTitle << "Game Engine Final!  Camera: "
            << ::cameraEye.x << ", "
            << ::cameraEye.y << ", "
