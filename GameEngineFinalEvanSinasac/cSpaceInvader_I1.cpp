...
cSpaceInvader_I1::cSpaceInvader_I1()
{
...
void cSpaceInvader_I1::Update(double deltaTime)
{
	if (::canWeMove)
	{
		bombTimer -= deltaTime;
...
std::string cSpaceInvader_I1::getName(void)
{
	return this->myShipName;
}

cMesh* cSpaceInvader_I1::getMeshPointer(void)
{
...
bool cSpaceInvader_I1::SetReciever(iMediator* pTheReciever)
{
...
bool cSpaceInvader_I1::RecieveMessage(sMessage theMessage)
{
		...
		for (unsigned int y = 0; y < Y; y++)
		{
			for (unsigned int x = 0; x < X; x++)
			{
				...
				if (this->blockPose1[x][y] == "X")
				{
					...
					pSolid->wholeObjectDiffuseRGBA = glm::vec4((102.0f / 255.0f), 1.0f, 1.0f, 1.0f);
...
bool cSpaceInvader_I1::RecieveMessage(sMessage theMessage, sMessage& messageReply)
{
	return true;
}

/*static*/ const std::string cSpaceInvader_I1::SPACE_INVADER_SHIPTYPE_AS_STRING = "I1";

std::string cSpaceInvader_I1::getShipTypeAsString(void)
{
	return cSpaceInvader_I1::SPACE_INVADER_SHIPTYPE_AS_STRING;
}

unsigned int cSpaceInvader_I1::getShipTypeAsUInt(void)
{
	return cSpaceInvader_I1::SPACE_INVADER_SHIPTYPE_AS_UINT;
}

// *******************************************************************
// From the iPhysicalObject interface
glm::vec3 cSpaceInvader_I1::getPosition(void)
{
	return this->m_PhysicalProperties.position;
}

void cSpaceInvader_I1::setPosition(const glm::vec3& newPosition)
{
	this->m_PhysicalProperties.position = newPosition;
	return;
}

glm::vec3 cSpaceInvader_I1::getVelocity(void)
{
	return this->m_PhysicalProperties.velocity;
}

void cSpaceInvader_I1::setVelocity(const glm::vec3& newVelocity)
{
	this->m_PhysicalProperties.velocity = newVelocity;
	return;
}

glm::vec3 cSpaceInvader_I1::getAcceleration(void)
{
	return this->m_PhysicalProperties.acceleration;
}

void cSpaceInvader_I1::setAcceleration(const glm::vec3& newAcceleration)
{
	this->m_PhysicalProperties.acceleration = newAcceleration;
	return;
}

glm::vec3 cSpaceInvader_I1::getOrientationEuler(void)
{
	return this->m_PhysicalProperties.orientationEuler;
}

void cSpaceInvader_I1::setOrientationEuler(const glm::vec3& newOrientationEuler)
{
	this->m_PhysicalProperties.orientationEuler = newOrientationEuler;
	return;
}

glm::quat cSpaceInvader_I1::getOrientationQuaternion(void)
{
	return this->m_PhysicalProperties.orientationQuat;
}

void cSpaceInvader_I1::setOrientationQuaternion(const glm::quat& newOrientationQuaternion)
{
	this->m_PhysicalProperties.orientationQuat = newOrientationQuaternion;
	return;
}

sPhysicalProps cSpaceInvader_I1::getPhysicalProperties(void)
{
	return this->m_PhysicalProperties;
}

void cSpaceInvader_I1::setPhysicalProperties(const sPhysicalProps& newPhysProps)
{
...
bool cSpaceInvader_I1::loadBlockPose1(std::string fileName)
{
	std::stringstream ss;
...
bool cSpaceInvader_I1::loadBlockPose2(std::string fileName)
{
	std::stringstream ss;
	//ss << SOLUTION_DIR << "\\GameEngineFinalEvanSinasac\\";
	ss << fileName;
...
bool cSpaceInvader_I1::loadExplosion1(std::string fileName)
{
	std::stringstream ss;
	//ss << SOLUTION_DIR << "\\GameEngineFinalEvanSinasac\\";
	ss << fileName;
...
bool cSpaceInvader_I1::loadExplosion2(std::string fileName)
{
	std::stringstream ss;
	//ss << SOLUTION_DIR << "\\GameEngineFinalEvanSinasac\\";
	ss << fileName;
